...
export const SupervisorPageDayOffAPIContextProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        const getRequests = async () => {
            try {
                const response = await axios.get(`http://34.75.226.10:80/day-off/get-all-requests/${isAuthenticated.token}`)
    ...
    const handleDayOffDecision = async (requestId, decision) => {
        ...
        try {
            const response = await axios.patch("http://34.75.226.10:80/day-off/update-request", payload)
