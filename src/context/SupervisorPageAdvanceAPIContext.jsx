...
export const SupervisorPageAdvanceAPIContextProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        const getRequests = async () => {
            try {
                const response = await axios.get(`http://34.75.226.10:80/advance/get-all-requests/${isAuthenticated.token}`)
    ...
    const handleAdvanceDecision = async (requestId, decision) => {
        ...
        try {
            const response = await axios.patch("http://34.75.226.10:80/advance/update-request", payload)
