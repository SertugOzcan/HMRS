...
export const PersonnelPageAdvanceAPIContextProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        const getRequests = async () => {
            try {
                const response = await axios.get(`http://34.75.226.10:80/advance/get-all-my-requests/${isAuthenticated.token}`)
    ...
    const handleSubmit = async (newRequest) => {
        ...
        try {
            const response = await axios.post("http://34.75.226.10:80/advance/create-request", payload)
    ...
    const handleCancelRequest = async (requestId) => {
        ...
        try {
            const response = await axios.patch("http://34.75.226.10:80/advance/cancel-request", payload)
            if (response.status === 200) {
                window.location.reload(true);
            }    
        } catch (error) {
